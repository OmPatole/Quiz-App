...
const ActivityHeatmap = ({ activityMap }) => {
    ...
    const getColor = (item) => {
        ...
        if (item.count <= 1) return 'bg-blue-900/60 border border-blue-900';
        if (item.count <= 3) return 'bg-blue-700/80 border border-blue-700';
        if (item.count <= 5) return 'bg-blue-600 border border-blue-500';
        return 'bg-blue-500 border border-blue-400 shadow-[0_0_8px_rgba(59,130,246,0.5)]';
    };
        ...
        <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-xl mb-8 font-sans flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1 overflow-hidden">
                ...
                <div className="flex items-center gap-1.5 mt-4 text-[10px] text-neutral-500 justify-end">
                    ...
                    <div className="w-3 h-3 rounded-[2px] bg-blue-900/60 border border-blue-900"></div>
                    <div className="w-3 h-3 rounded-[2px] bg-blue-700/80 border border-blue-700"></div>
                    <div className="w-3 h-3 rounded-[2px] bg-blue-600 border border-blue-500"></div>
                    <div className="w-3 h-3 rounded-[2px] bg-blue-500 border border-blue-400"></div>
            ...
            <div className="flex flex-row md:flex-col gap-2 md:pl-6 md:border-l border-neutral-800/50 text-xs font-bold text-neutral-500 min-w-[80px]">
                {availableYears.map(year => (
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-lg transition-all text-left w-full ${year === selectedYear ? 'bg-blue-500/10 text-blue-400 border border-blue-300 dark:border-blue-500/30' : 'hover:bg-neutral-800 hover:text-gray-700 dark:hover:text-neutral-300 border border-transparent'}`}
                    >
                        {year}
                    </button>
                ))}
    ...
    return (
        <div className="animate-in fade-in slide-in-from-right-4 duration-300 pb-20 w-full">
            ...
            <div className="flex items-center gap-4 mb-8">
                {onBack && (
                    <button onClick={onBack} className="w-10 h-10 rounded-full bg-neutral-900 border border-neutral-800 flex items-center justify-center text-neutral-400 hover:text-gray-900 dark:hover:text-white hover:border-gray-400 dark:hover:border-neutral-600 transition"><ArrowLeft size={18} /></button>
                )}
                <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-xl flex items-center gap-4 hover:border-gray-300 dark:hover:border-neutral-700 transition">
                    <div className="w-12 h-12 rounded-full bg-blue-900/20 flex items-center justify-center text-blue-400"><CheckSquare size={24} /></div>
                    <div>
